...
def make_decision(joint=True, composite=True, guardrail_fail=False, notes=None):
    ...
    metrics.loss = NS(
        expected_loss={"control": 0.0, "B": 0.01, "C": 0.02}, control="control"
    )
    ...
    guardrails = NS(
        ...
        variant_passed={"control": True, "B": not guardrail_fail, "C": False},
        guardrails=[],
        conflicts=[],
    )
    joint_obj = None
    ...
    if composite:
        composite_obj = NS(
            ...
            metric_contributions={"B": {"primary": 0.3}, "C": {"primary": -0.1}},
            best_variant="B",
            threshold=0.0,
        )
...
def make_segment_decision(
    best="B", state="strong win", recommendation="ship variant", guardrail_pass=True
):
    ...
    metrics.lift = NS(
        mean={"control": 0.0, "B": 0.05},
        hdi_low={"control": 0.0, "B": 0.02},
        hdi_high={"control": 0.0, "B": 0.08},
        hdi_prob=0.94,
    )
    metrics.rope = NS(
        inside_rope={"control": 1.0, "B": 0.2},
        outside_rope={"control": 0.0, "B": 0.8},
        prob_practical={"control": 0.0, "B": 0.8},
    )
    failed_guardrail = NS(
        metric="page_load",
        variant="B",
        passed=guardrail_pass,
        prob_degraded=0.0 if guardrail_pass else 0.92,
        threshold=0.1,
        severity="low" if guardrail_pass else "high",
        expected_degradation=0.0,
    )
    guardrails = NS(
        all_passed=guardrail_pass,
        variant_passed={"control": True, "B": guardrail_pass},
        guardrails=[failed_guardrail],
        conflicts=[],
    )
...
def make_hierarchical_results(violations=False):
    ...
    segment_results = {
        "mobile": make_segment_decision(
            state="strong win", recommendation="ship variant"
        ),
        "desktop": make_segment_decision(
            state="weak win", recommendation="consider shipping"
        ),
        "tv": make_segment_decision(
            state="inconclusive",
    ...
    return Results(
        aggregate,
        config={"rope_bounds": (-0.01, 0.01)},
        segment_results=segment_results,
        segment_guardrail_violations=seg_violations,
    )
...
class TestHierarchicalResults:
    ...
    def test_segment_summary_detects_conflict(self):
        ...
        segment_results = {
            ...
            "desktop": make_segment_decision(
                state="inconclusive", recommendation="do not ship"
            ),
        }
