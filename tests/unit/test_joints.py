...
class TestJointThresholds:
    def test_primary_threshold_blocks(self):
        ...
        primary, guardrail, names, control = make_basic_inputs()
        primary[:, 1] = 1.01
        res = compute_joint_probability(
            primary, guardrail, names, control, primary_threshold=0.05
        )
        assert res.joint_prob["B"] == 0.0
    ...
    def test_guardrail_threshold_allows(self):
        ...
        res = compute_joint_probability(
            primary, guardrail, names, control, guardrail_thresholds={"error": 0.1}
        )
        assert res.joint_prob["B"] == 1.0
...
class TestJointCorrelation:
    ...
    def test_negative_correlation_gap(self):
        ...
        res = compute_joint_probability(
            primary, guardrail, names, control, lower_is_better={"error": True}
        )
...
class TestJointValidation:
    ...
    def test_empty_metrics_to_join(self):
        ...
        with pytest.raises(ValueError):
            compute_joint_probability(
                primary, guardrail, names, control, metrics_to_join=[]
            )
    ...
    def test_unknown_metric(self):
        ...
        with pytest.raises(ValueError):
            compute_joint_probability(
                primary, guardrail, names, control, metrics_to_join=["fake"]
            )
...
class TestJointWarnings:
    ...
    def test_lower_is_better_warning(self):
        ...
        with pytest.warns(UserWarning):
            compute_joint_probability(
                primary, guardrail, names, control, lower_is_better=None
            )
...
class TestJointMetricSubset:
    def test_metrics_to_join_subset(self):
        ...
        primary = np.column_stack([control, variant])
        guardrail = {
            "good_metric": np.column_stack(
                [np.array([1, 1, 1, 1]), np.array([0.5, 0.5, 0.5, 0.5])]
            ),
            "bad_metric": np.column_stack(
                [np.array([1, 1, 1, 1]), np.array([2, 2, 2, 2])]
            ),
        }
        names = ["control", "B"]
        res = compute_joint_probability(
            primary, guardrail, names, "control", metrics_to_join=["good_metric"]
        )
        assert res.joint_prob["B"] == 1.0
