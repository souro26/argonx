...
def make_basic_inputs():
    ...
    primary = np.array([[1.0, 1.2], [1.0, 1.3], [1.0, 1.1]])
    guardrails = {"error": np.array([[0.1, 0.08], [0.1, 0.07], [0.1, 0.09]])}
    names = ["control", "B"]
...
class TestThreshold:
    def test_prob_exceeds_threshold(self):
        ...
        primary, guardrails, names, weights, gb = make_basic_inputs()
        res = compute_composite_score(
            primary, guardrails, names, "control", weights, gb, threshold=0.0
        )
    ...
    def test_gap_distribution_centered(self):
        ...
        primary, guardrails, names, weights, gb = make_basic_inputs()
        res = compute_composite_score(
            primary, guardrails, names, "control", weights, gb, threshold=0.1
        )
    ...
    def test_gap_hdi_positive_when_far_above_threshold(self):
        ...
        primary = np.array([[1.0, 2.0], [1.0, 2.0], [1.0, 2.0]])
        guardrails = {}
        names = ["control", "B"]
        weights = {"primary": 1.0}
        gb = MockGuardrails({"control": True, "B": True})
        res = compute_composite_score(
            primary, guardrails, names, "control", weights, gb, threshold=0.5
        )
        low, _ = res.gap_hdi["B"]
        assert low > 0
...
class TestGuardrails:
    def test_penalty_applied_when_failed(self):
        ...
        res = compute_composite_score(
            primary, guardrails, names, "control", weights, gb, guardrail_penalty=1.0
        )
        assert res.score["B"] < 0
    ...
    def test_penalty_zero_when_all_pass(self):
        ...
        primary, guardrails, names, weights, gb = make_basic_inputs()
        res = compute_composite_score(
            primary, guardrails, names, "control", weights, gb, guardrail_penalty=10.0
        )
        assert res.score["B"] > 0
...
class TestWeights:
    def test_missing_weights_raises(self):
        ...
        with pytest.raises(ValueError):
            compute_composite_score(primary, guardrails, names, "control", {}, gb)
    ...
    def test_invalid_weight_keys_raises(self):
        ...
        with pytest.raises(ValueError):
            compute_composite_score(
                primary, guardrails, names, "control", {"invalid": 1.0}, gb
            )
    ...
    def test_asymmetric_deterioration_weights(self):
        ...
        primary = np.array([[1.0, 0.8], [1.0, 0.7], [1.0, 0.9]])
        guardrails = {}
        names = ["control", "B"]
        weights = {"primary": 1.0}
        gb = MockGuardrails({"control": True, "B": True})
...
class TestMultiVariant:
    def test_three_variants_ranking(self):
        ...
        primary = np.array([[1.0, 1.2, 0.9], [1.0, 1.3, 0.8], [1.0, 1.25, 0.85]])
        guardrails = {}
        names = ["control", "B", "C"]
        weights = {"primary": 1.0}
        gb = MockGuardrails({"control": True, "B": True, "C": True})
        res = compute_composite_score(
            primary, guardrails, names, "control", weights, gb
        )
        assert res.best_variant == "B"
...
class TestEdgeCases:
    ...
    def test_all_variants_below_threshold(self):
        ...
        primary, guardrails, names, weights, gb = make_basic_inputs()
        res = compute_composite_score(
            primary, guardrails, names, "control", weights, gb, threshold=100.0
        )
...
class TestMathematicalProperties:
    ...
    def test_exact_asymmetric_deterioration(self):
        ...
        res = compute_composite_score(
            ...
            weights,
            gb,
            deterioration_weights=deterioration_weights,
        )
