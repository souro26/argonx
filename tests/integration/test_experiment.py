    ...
    return pd.DataFrame(
        {
            ...
            "revenue": np.concatenate(
                [
                    np.random.lognormal(3.8, 0.6, n),
                    np.random.lognormal(3.8 + lift_b, 0.6, n),
                ]
            ),
    ...
    return pd.DataFrame(
        {
            ...
            "converted": np.concatenate(
                [np.random.binomial(1, 0.1, n), np.random.binomial(1, 0.14, n)]
            ),
        }
    )
...
def make_df_segmented(n_per_segment=150):
    ...
    for seg in segments:
        ...
        for variant, mu in [("control", 3.8), ("variant_b", 3.88)]:
    ...
    def test_rejects_missing_guardrail_column(self):
        ...
        with pytest.raises(ValueError):
            Experiment(
                ...
                primary_metric="revenue",
                model="lognormal",
                guardrails=["nonexistent"],
            )

    def test_rejects_lower_is_better_not_in_guardrails(self):
        """Rejects lower_is_better key not a declared guardrail."""
        df = make_df_lognormal()
        with pytest.raises(ValueError):
            Experiment(
                df,
        ...
        df = pd.DataFrame(
            {
                ...
                "latency": np.concatenate(
                    [np.random.normal(100, 10, N), np.random.normal(108, 10, N)]
                ),
            }
        )
        ...
        df = pd.DataFrame(
            {
                ...
                "purchases": np.concatenate(
                    [
                        np.random.poisson(5, N).astype(int),
                        np.random.poisson(6, N).astype(int),
                    ]
                ),
            }
        )
...
class TestFlatResultStructure:
    ...
    def test_guardrail_passes_when_improving(self):
        ...
        result = exp.run(
            n_draws=200, config={"guardrail_thresholds": {"page_load": 0.1}}
        )
        load_results = [
            g for g in result.guardrails.guardrails if g.variant == "variant_b"
        ]
        assert load_results[0].passed is True

    def test_composite_score_computed(self):
        """Composite score computed when composite_weights provided."""
        df = make_df_lognormal()
        exp = Experiment(
            df,
            "variant",
        ...
        result = exp.run(
            ...
            config={"guardrail_thresholds": {"page_load": 0.1}},
        )
...
class TestHierarchicalModelSelection:
    ...
    def test_hierarchical_with_guardrail(self):
        """Hierarchical run with guardrail produces segment results."""
        df = make_df_segmented()
        exp = Experiment(
            df,
            "variant",
    ...
    def test_to_dataframe_hierarchical_has_multiindex(self):
        """to_dataframe returns MultiIndex (segment, variant)."""
        df = make_df_segmented()
        exp = Experiment(
            df,
            "variant",
