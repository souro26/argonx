...
class _HierarchicalBase(BaseModel):
    """
    Internal base class shared by HierarchicalGaussianModel and
    HierarchicalStudentTModel. Not part of the public API.

    Handles all shared logic: nested data validation, pre-flight checks,
    parameterization selection, health checks, tau collapse detection,
    posterior width checks, segment extraction, and population marginalisation.
    """
    ...
    def _build_likelihood(
        ...
        mu_seg: object,
        sigma: object,
        obs_arrays: list[list[np.ndarray]],
    ) -> None:
        """
        Add likelihood nodes to the active PyMC model.

        Subclasses override this to plug in Normal or StudentT likelihood.
        Called inside a `with pm.Model()` block in _run_mcmc().
        """
    ...
    def _run_mcmc(self, n_draws: int) -> az.InferenceData:
        ...
        with pm.Model():
            ...
            self._build_likelihood(mu_seg, sigma, obs_arrays)
    ...
    def _extract_by_segment(self, trace: az.InferenceData, n_draws: int) -> np.ndarray:
        ...
        flat = mu_vals.reshape(total_draws, mu_vals.shape[2], mu_vals.shape[3])
...
class HierarchicalGaussianModel(_HierarchicalBase):
    ...
    def _build_likelihood(self, mu_seg, sigma, obs_arrays):
        for i in range(len(obs_arrays)):
            for j in range(len(obs_arrays[i])):
                pm.Normal(
                    f"obs_{i}_{j}",
                    mu=mu_seg[i, j],
                    sigma=sigma,
